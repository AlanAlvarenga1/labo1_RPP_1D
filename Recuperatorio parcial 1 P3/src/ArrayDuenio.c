...
void hardCodearDuenios (sDuenio *lista, int cantHardcodear,int* cuentaDuenios,int* idFinal) {
	...
	char nombre[][41]= {"Alan","alan","Lucia","lucia","Federico"};
...
int chequearEstadiaAlan (sDuenio* lista,int max) {
	...
	for (i=0;i<max;i++) {
		if (((strcmp(lista[i].nombre,"Alan")==0) || strcmp(lista[i].nombre,"alan")==0) && lista[i].cantEstadias>0) {
			cantidadVerificada++;
		}


		/*if ((lista[i].nombre=="Alan" || lista[i].nombre=="alan") && lista[i].cantEstadias>0) {
			cantidadVerificada++;
		}*/
	}

	if (cantidadVerificada>0) {
		retorno=cantidadVerificada;
	}
